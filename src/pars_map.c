...
/*   Updated: 2024/12/10 18:20:11 by sadegrae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	temp_texture(t_data *game, int i, int j, int start)
{
	...
	free(tmp);
	if (ft_strlen(game->recup.f[game->k]) >= 3)
	{
		printf("Error color");
		exit(1);
	}
	if (ft_strlen(game->recup.f[game->k]) == 1)
	{
...
void	temp_texture_wall(t_data *game, int i, int j, int start)
{
	...
	free(tmp);
	if (ft_strlen(game->recup.c[game->k]) >= 3)
	{
		printf("Error color");
		exit(1);
	}
	if (ft_strlen(game->recup.c[game->k]) == 1)
	{
		tmp = malloc(sizeof(char) * 3);
		if (!tmp)
			return ;
